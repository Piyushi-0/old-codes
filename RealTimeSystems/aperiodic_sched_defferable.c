/*
C program for Aperiodic Scheduling using Defferable Server in RTS
*/
...
int es1=ti+ps+sqrt( pow((ti+ps),2) -4*ps*(ti-t[j].e-sum) ), es2=ti+ps-
sqrt( pow((ti+ps),2) -4*ps*(ti-t[j].e-sum) );//printf("%d\n%d\n",es1,es2);
if(es1>0 && es1<ps)
	{
	if(es2>0 && es2<ps)
		{
...
int edf(int time,struct s t[])
{
	...
	for(i=0;i<n;i++)
		...
		if(t[i].arv[t[i].lx+1]<=time)//next instance remaining has arrived
			...
			if(i==n)//server
				{
				if(ar>time || ae==0 || es==0)//aperiodic not arrived or completed or no
				budget
				continue;
				}
			if(minx==-1)
...
printf("Enter period, execution time of server\n");
scanf("%d%d",&ps,&es);
t[n].p=t[n].d=ps;
t[n].ex=t[n].rex=es;
t[n].lx=-1;
